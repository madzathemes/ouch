...
function ouch_css() {
	...
	wp_enqueue_style('ouch-style', get_stylesheet_uri());

	$custom_styles = '';
	$options = get_option("ouch_theme_options");
	...
	if(!empty($options['colors_default'])){
	 ...
	 if ( $custom_styles != '' ) {
	  $css = preg_replace("/(^[\r\n]*|[\r\n]+)[\s\t]*[\r\n]+/", "\n", $custom_styles);
		wp_add_inline_style( 'ouch-style', $css );
	}

}
add_action( 'wp_enqueue_scripts', 'ouch_css');

function ouch_header_script() {


		$option = get_option("ouch_theme_options");

		wp_enqueue_script( 'ouch_script', get_template_directory_uri(). '/inc/js/scripts.js', array( 'jquery'), '', true );
		wp_localize_script( 'ouch_script', 'ajax_posts', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ), 'noposts' => esc_html__('No older posts found', 'ouch'), ));
    ...
    function ouch_fonts_url() {
      ...
      $theme_font = "Lato:400,900,700";
         ...
         */
        if ( 'off' !== _x( 'on', 'Google font: on or off', 'ouch' ) ) {
            $font_url = add_query_arg( 'family', urlencode( ''. esc_attr($theme_font) .'' ), "//fonts.googleapis.com/css" );
        }
        return $font_url;
    }
    wp_enqueue_style( 'ouch-fonts', ouch_fonts_url(), array(), '1.0.0' );
}
add_action('wp_enqueue_scripts', 'ouch_header_script');




function ouch_admin_script() {
	wp_enqueue_style('ouch-admin', get_template_directory_uri().'/inc/css/admin.css');
}
add_action('admin_enqueue_scripts', 'ouch_admin_script');


add_filter('body_class','ouch_class');
function ouch_class($classes) {
