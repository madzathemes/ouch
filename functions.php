...
function ouch_theme_setup() {
	...
	load_theme_textdomain( 'ouch', get_template_directory() . '/languages' );
	$locale = get_locale();
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Header Menu', 'ouch' ),
		'mobile' => esc_html__( 'Mobile Menu', 'ouch' ),
		'footer_menu' => esc_html__( 'Footer Navigation', 'ouch' ),
	) );
	if ( ! isset( $content_width ) ) { $content_width = 900; }
}

add_action( 'after_setup_theme', 'ouch_theme_setup' );
...
function ouch_javascript_detection() {
	echo "<script>(function(html){html.className = html.className.replace(/\bno-js\b/,'js')})(document.documentElement);</script>\n";
}
add_action( 'wp_head', 'ouch_javascript_detection', 0 );
 ...
 * @return boolean
 */
function ouch_disbale_wpes_on_media_search($enabled) {
...
add_filter('wpes_enabled', 'ouch_disbale_wpes_on_media_search');
