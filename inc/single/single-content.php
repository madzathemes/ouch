...
function ouch_single_bottom() {
 ...
 ?><div class="mt-pagepagination">
  <?php $defaults = array(
		'before'           => '<p>' . esc_html__( 'Read More:', 'ouch'),
		'after'            => '</p>',
		'link_before'      => '<span>',
		'link_after'       => '</span>',
  ...
  wp_link_pages( $defaults ); ?>
     ...
     <?php if ( get_the_author_meta( 'description' )) { ?>
      <div class="author-info">
        <div class="author-avatar">
          <?php echo get_avatar( get_the_author_meta( 'user_email' ), apply_filters( 'ouch_author_bio_avatar_size', 55 ) ); ?>
          </div>
          <div class="author-description">
            <h5><?php echo esc_html__( 'Author', 'ouch' ); ?></h5>
            <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author"><h3><?php printf( esc_html__( '%s', 'ouch' ), get_the_author() ); ?></h3></a>
          ...
          <div class="author-link">
            <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author">
              <?php printf( esc_html__( 'View all posts by %s', 'ouch' ), get_the_author() ); ?>
            </a>
          </div>
        </div>
      </div>
   ...
   <?php if ( is_active_sidebar( 'sidebar-single-bottom-widget-area-before' ) ) {
     ...
     dynamic_sidebar( 'sidebar-single-bottom-widget-area-before' );
      ...
      if ( shortcode_exists( 'posts' ) ) { echo do_shortcode('[posts title="'. esc_html__( 'You may be interested','ouch' ) .'" title_type="left" type=normal-right-small item_nr=3 ]'); }
